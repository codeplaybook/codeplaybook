...
Re-deploy `.codeplaybook/` standards to the agent's rules directory without re-analyzing the codebase.
...
- After editing `.codeplaybook/standards/*.md`
- After deleting a standard from `.codeplaybook/`
...
Check that `.codeplaybook/` has at least one standard:
...
```
No standards found in .codeplaybook/.
...
Remove previously deployed codeplaybook files from `$AGENT_RULES_DIR` to avoid stale artifacts. Only remove files with the `codeplaybook-` prefix to avoid disturbing other agent configuration.
...
Ensure the generated rules are surfaced in the agent's main configuration file.
