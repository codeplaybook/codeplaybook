...
```yaml
name: "codeplaybook-extract-responsibility"
summary: "Extract misplaced logic into the correct architectural layer when a file mixes responsibilities"
whenToUse:
  - "A controller contains business logic that should be in a service"
  - "A service makes direct database calls that should be in a repository"
contextValidationCheckpoints:
  - "Which file has the mixed responsibility?"
  - "What layer should the extracted logic move to?"
steps:
  - name: "Identify the misplaced code"
    description: "Find the logic that doesn't belong in this file's architectural role"
  - name: "Create or find the target file"
    description: "Create a new service/repository/handler or find an existing one"
  - name: "Move the logic"
    description: "Extract the code to the target, replace with a delegation call"
  - name: "Update tests"
    description: "Move or create tests for the extracted logic"
```